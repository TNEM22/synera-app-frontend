        ...
        axios
          ...
          .catch((err) => {
            ...
            if (err.response.status === 404) {
              ...
              setTimeout(() => setHighlightState(false), 3000);
            } else {
        ...
        <form onSubmit={handleFormSubmit} className="relative space-y-4">
          ...
          >
            <Link
              to="/signup"
              className={
                'block text-center font-bold mt-3 py-2 rounded-md cursor-pointer text-white bg-indigo-500 active:bg-indigo-600 shadow-md hover:shadow-indigo-500 ' +
                (highlightState && 'animate-bounce')
              }
            >
              Sign Up
            </Link>
