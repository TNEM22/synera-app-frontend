...
import ClientErrorPage from '../../pages/ClientErrorPage/ClientErrorPage';
...
const NavigationPanel = () => {
  ...
  const hasEntered = useAppSelector((state) => state.navigation.hasEntered);
  ...
  const navigationElements: { [key in NavigationKeys]: NavigationElements } = {
    '/': {
      id: 'home',
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        dispatch(setUserPhoto(dd.photo));
        dispatch(setUserEmail(dd.email));
        dispatch(setUserName(dd.name));
        dispatch(setUserRole(dd.role));
      })
      .catch((err) => {
    ...
    document
      .querySelector('html')
      ?.classList.add(localStorage.theme || 'light');
      ...
      {isError ? (
        ...
        >
          <div>
            ...
            >
              {(Object.keys(navigationElements) as NavigationKeys[]).map(
                (key) => {
